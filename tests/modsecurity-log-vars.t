...
# Tests for $modsecurity_intervention, $modsecurity_deny and
...
http {
    ...
    log_format modsec '$request_uri|i=$modsecurity_intervention|d=$modsecurity_deny|r=$modsecurity_triggered_rules';
    ...
    server {
        ...
        location /status-only {
            ...
            modsecurity_rules '
                SecRuleEngine On
                SecRule ARGS "@streq go" "id:450,phase:1,log,pass,status:403"
            ';
        }
...
# No rule matches: intervention=0, deny=0, no rule list (nginx prints '-' for missing var).
http_get('/pass?arg=x');
like(log_line($t, '/pass'), qr/\|i=0\|/,      'pass: intervention=0');
like(log_line($t, '/pass'), qr/\|d=0\|/,      'pass: deny=0');
like(log_line($t, '/pass'), qr/\|r=-$/,       'pass: no triggered rules');

# Rule matches but non-disruptive: intervention=0, deny=0, rule id listed.
http_get('/match-logonly?arg=hit');
like(log_line($t, '/match-logonly'), qr/\|i=0\|/,  'log-only: intervention=0');
like(log_line($t, '/match-logonly'), qr/\|d=0\|/,  'log-only: deny=0');
like(log_line($t, '/match-logonly'), qr/\|r=200$/, 'log-only: rule id captured');

# Multiple rules all matching: intervention=0, deny=0, every id listed.
http_get('/multi?arg=hit');
like(log_line($t, '/multi'), qr/\|i=0\|/,               'multi: intervention=0');
like(log_line($t, '/multi'), qr/\|d=0\|/,               'multi: deny=0');
...
like(log_line($t, '/mixed-logging'), qr/\|d=0\|/,   'mixed-logging: deny=0');
...
like(log_line($t, '/allow'), qr/\|d=0\|/,   'allow: deny=0');
like(log_line($t, '/allow'), qr/\|r=800$/,  'allow: only the allow rule captured; subsequent deny short-circuited');

# Deny intervention (phase 1): intervention=1, deny=1, rule id listed.
http_get('/block?arg=go');
like(log_line($t, '/block'), qr/\|i=1\|/,    'block: intervention=1');
like(log_line($t, '/block'), qr/\|d=1\|/,    'block: deny=1');
like(log_line($t, '/block'), qr/\|r=400$/,   'block: rule id captured');

# status action alone (with pass) only records a response code; it is NOT
# disruptive, so neither intervention nor deny fires.
http_get('/status-only?arg=go');
like(log_line($t, '/status-only'), qr/\|i=0\|/,  'status-only: intervention=0 (status alone is not disruptive)');
like(log_line($t, '/status-only'), qr/\|d=0\|/,  'status-only: deny=0');
like(log_line($t, '/status-only'), qr/\|r=450$/, 'status-only: rule id captured');

# Redirect intervention: intervention=1 but deny=0 (redirect is not a block).
http_get('/redirect?arg=go');
like(log_line($t, '/redirect'), qr/\|i=1\|/,  'redirect: intervention=1');
like(log_line($t, '/redirect'), qr/\|d=0\|/,  'redirect: deny=0 (redirect is not a block)');
...
like(log_line($t, '/block-phase3'), qr/\|d=1\|/,  'phase3 block: deny=1');
